...
void TMVAClassificationApplication(TString myMethodList = "")
{
   ...
   //
    TString inputName = getenv("CURRENT_WS2_DATASET");
    int ext_length = ((TString) ".root").Length();
    TString outputName(inputName);
    outputName.Insert(inputName.Length() - ext_length, "_TMVA2");

    std::cout << "[INFO] Input File: " << inputName << std::endl
              << "[INFO] Output File: " << outputName << std::endl;


   TFile *input = new TFile(inputName);
   TFile *target = new TFile(outputName, "RECREATE");

   std::cout << "--- Select signal sample" << std::endl;
   TTree *theTree = ((TTree *)input->Get("DecayTree"))->CloneTree();
   TTree *mva_tree = new TTree("mva_response", "mva_response");
   theTree->AddFriend(mva_tree);
   Double_t MVA_LD,
       MVA_MLP,
       MVA_MLPBNN,
       MVA_DNN_CPU,
